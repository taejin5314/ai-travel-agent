...
export async function planTrip(
  ...
  for (const rawName of preferences.mustVisit) {
    ...
    const exact = matches.find((p) => normalize(p.name) === normalize(rawName));
  ...
  const interestPlaces = catalog.filter(
    (p) => !queuedIds.has(p.id) && interests.has(p.category),
  );
  ...
  const check = validateItinerary(itinerary, catalog, preferences);
  if (!check.ok) {
    return {
      ok: false,
